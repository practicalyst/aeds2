...
// Critério de desempate em nome
int comparePlayers(jogador *a, jogador *b) {
    if (a->peso != b->peso) {
...
// Estou passando a variavel global como parametro para o caso dessa função ser reutilizada uma outra hora
void sortShellSort(jogador *j[], int qtdJogadores, int *cmp, int *mov) {
    int i, z;
    int h = 1;

    while (h < qtdJogadores) {
        h = 3 * h + 1;
        (*cmp)++;
    }

    while (h > 0) {
        for (i = h; i < qtdJogadores; i++) {
            jogador * temp = j[i];
            z = i;

            while (z > h - 1 && comparePlayers(j[z - h], temp) > 0){
                j[z] = j[z - h];
                z = z - h;
                (*mov)++;
                (*cmp)++;
...
void main(){
    ...
    sortShellSort(novosJogadores, qtdJogadores, &cmp, &mov);
    clock_t tempoFinal = clock();
    clock_t tempo = tempoFinal - tempoInicial;
    registrarLog("matrícula_shellsort.txt", "790871", tempo, cmp, mov);
